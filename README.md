...
* 音声ミキシング機能
* YouTubeアップロード機能
* アプリ会員限定YouTubeチャンネルとの連携機能（会員限定チャンネル連携機能実装済、個人チャンネル投稿も対応予定 ※現在Google側で権限取得中）
* エコー・リバーブなどの音声エフェクト機能
* 動画書き出し機能（録音と伴奏を合成した動画生成）
* パスワードリセット・更新機能
* 静的OGP実装
* YouTube動画検索オートコンプリート機能
* 独自ドメイン取得
...
音声処理: MediaRecorder API / Web Audio API（リバーブ・ミキシング実装済）
外部サービス連携: YouTube iframe API / YouTube Data API
ストレージ: Amazon S3（本番環境）
